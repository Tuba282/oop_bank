...
async function main() {
    ...
    async function start() {
        await new Promise((resolve) => {
            figlet.text(`Welcome to Bank`, { font: "Slant" }, function (err, data) {
                if (err) {
    ...
    await start();
    // ------------------------------create-bank-balance------------------------------
    let bank_balance = 50000;
    // ------------------------------getting-customer-details------------------------------
    let Answer_details = await inquirer.prompt([
        {
            name: "name",
            type: "string",
            message: `\nEnter your name \n${chalk.cyan(`(less than or equal to 11 letters)`)}:`,
        },
        {
            name: "account_number",
            type: "number",
            message: `\nEnter your 6 digit account number.\n${chalk.cyan("recommanded 454545")}`,
        },
    ]);
    // ------------------------------show varification start message------------------------------
    await figlet(`varification Start`, { font: "Slant", width: 100 }, function (err, data) {
        if (err) {
    ...
    // -------------------------------varification------------------------------
    if (Answer_details.name.length < 12 &&
        Answer_details.account_number === 454545) {
        const spinner_1 = ora(" 🤚🏻 Please Wait for sec\nIt is Varification checking...").start();
        await new Promise((resolve) => setTimeout(() => {
            resolve(spinner_1.succeed("And your are Verified! 👍🏻"));
            console.log(`\nHello ${chalk.blueBright(Answer_details.name)} your varification is completed
                \nYour account number is ${chalk.blueBright(Answer_details.account_number)}
        ...
        async function asking() {
            ...
            // -------------------------------Current-Account------------------------------
                ...
                if (current_account.list === "Withdraw") {
                    ...
                    const spinner_2 = ora("\n Please wait for little moment 🤏🏻\n\n we are working on your Withdrawal 📜").start();
                    await new Promise((resolve) => setTimeout(() => {
                    ...
                    //
                    // -------------------------------adding little spinner for withdrawal ------------------------------
                    const spinner_2 = ora("\n Please wait for little moment 🤏🏻\n\n we are working on your Deposit 📜").start();
                    await new Promise((resolve) => setTimeout(() => {
            ...
            // -------------------------------Transfer-Money------------------------------
                ...
                await figlet(`Tranfer varification`, { font: "Slant", width: 100 }, function (err, data) {
                    if (err) {
                        console.dir(`Oops something went wrong!!!`);
                        console.log(err);
                    }
                ...
                // ------------------------------Transfer varification message------------------------------
                const spinner_3 = ora(`🤚🏻 Please Wait for sec\nIt is Transfer Varification checking...`).start();
                await new Promise((resolve) => setTimeout(() => {
                    resolve(spinner_3.succeed("And your are Transfer Verification is clear! 👍🏻"));
                }, 3000));
                // ------------------------------Transfer person message------------------------------
                console.log(`\nHello ${chalk.blueBright(Transfer_details.name)} your varification is completed
                \nYour account number is ${chalk.blueBright(Transfer_details.account_number)}
                \nAnd ${chalk.blueBright(Answer_details.name)} send Rs ${chalk.blueBright(Transfer_details.amount)} to your account.`);
                console.log(`\n${chalk.green("\n\nThank you for visiting our Bank 🫶🏻")} `);
                console.log(`\n${chalk.yellow(Transfer_details.name)} said: Hey ${chalk.blueBright(Answer_details.name)} 
